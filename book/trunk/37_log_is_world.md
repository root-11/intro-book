...
The structural shape is what carries: triple-store + codebook + double-buffered writer. That Rust analogue is built: [`code/logger`](../../code/logger) is a dependency-free crate with the same triple-store, evolving string codebook, `f64` type inference, and double-buffered revolver, writing raw little-endian column bytes ([§36](36_persistence_is_serialization.md)) instead of `.npz` - the bytes on disk are the bytes in memory. On this machine its `log()` costs ~160 ns at 5 fields and ~310 ns at 11 (`cargo run --release --bin benchmark`), roughly **6× the Python simlog** on the same workload, because the compiled path pays no interpreter dispatch and no per-element boxing. (The one piece left out is `to_sqlite`: SQLite is not in Rust's std, and a crate would forfeit the dependency-free property; it is a post-processing export, not part of the live log path.) Three views of the same idea are sketched in the stretch exercise below.
