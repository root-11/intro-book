...
(These GPU figures are a cost model with assumed bus and launch numbers, not a measurement: the reference machines have only integrated GPUs, which share the memory channel and so inherit its ceiling rather than crossing a bus. The discrete-GPU case the model assumes is an open invitation - run [`code/heterogeneous`](https://github.com/root-11/intro-book/tree/main/code/heterogeneous) on yours and post the numbers; it is the one cell these machines cannot fill.)
